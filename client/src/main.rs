...
fn main() {
    ...
    // Spawn a thread to handle incoming and outgoing messages
    thread::spawn(move || loop {
        let mut buff = vec![0; MSG_SIZE];
        match client_clone.read_exact(&mut buff) {
            Ok(_) => {
                // Convert buffer to a String and filter out padding
                let msg = buff
                    .into_iter()
                    .take_while(|&x| x != 0)
                    .collect::<Vec<_>>();
                let msg = String::from_utf8(msg).expect("Invalid UTF-8 message received");
